...
def policyPlotReduced_v2(T,r,priorE0Arr, pC0E0Arr, tValues, dataPath, lines, argumentR, argumentP, minProb,mainPath, plottingPath):
    ...
    for cueVal in pC0E0Arr: # for each cue validity
        ...
        for pE0 in priorE0Arr: # for each prior
            ...
            for t in tValues:
                ...
                aggregatedResultsDF['colors'] = np.array([color_palette[idx] for idx in aggregatedResultsDF['marker']])
                ...
                if t== 1:
                    subDF = aggregatedResultsDF[aggregatedResultsDF['time'] == 0]
                    subDF = subDF.reset_index(drop=True)
                    circles(np.array(subDF['time']), np.array(subDF['newpE1']),
                            s=np.array(subDF['area']), ax=ax, c=subDF['colors'], zorder=2, lw=0.7, alpha=alpha_val)

                #preapre the decision pies
                if t >= 1:
                    ...
                    if duplicateList:
                        ...
                        mask = subDF['newpE1'].isin(list(duplicateList.keys()))
                        newSubDf = subDF[~mask] #exclude the duplicates


                        circles(np.array(newSubDf['time']), np.array(newSubDf['newpE1']),
                                s=np.array(newSubDf['area']), ax=ax, c=newSubDf['colors'], zorder=2, lw=0.7, alpha=alpha_val)
                    else:
                        circles(np.array(subDF['time']), np.array(subDF['newpE1']),
                                s=np.array(subDF['area']), ax=ax, c=subDF['colors'], zorder=2, lw=0.7, alpha=alpha_val)
                ...
                for t in np.arange(0,T-1,1):
                    ...
                    for subIDX in range(len(subDF1)):
                        yArr = [[subDF1['newpE1'].loc[subIDX], subDF2['newpE1'].loc[yIDX]] for yIDX in
                                yvalsIDXAll[subIDX]]
                        [ax.plot(timeArr, yArrr, ls='solid', marker=" ", color='black', zorder=1, lw=0.3) for yArrr in
                         yArr]
                        del yArr
            ...
            for idx in range(len(coordinates)):
                ...
                #print(edgecolorPies[idx])

                #list of dictionaries for wedge properties
                # ,wedgeprops={'linewidth': 0.5, "edgecolor": edgecolorPies[idx],"alpha" :1}
                pp, tt = ax.pie(pieFracs, colors=colorsPies, radius=currentR, center=coordinates[idx], wedgeprops={"alpha":alpha_val})


                [p.set_zorder(3) for idy,p in enumerate(pp)] #+ len(coordinates) - idx
            ...
            plt.ylim(0.3, T - 1 + 0.7)
            plt.xlim(-0.85, T - 1 + 0.8)
    ...
    fig.tight_layout()
    fig.subplots_adjust(top=0.94, right=0.91)
    #plt.savefig(os.path.join(resultPath,'DevelopmentalTrajectory.pdf'), dpi = 900)
    plt.savefig(os.path.join(resultPath, 'DevelopmentalTrajectoryReduced.png'), bbox_inches='tight' ,dpi=600)



def policyPlotReduced_v2Paper(T,r,priorE0Arr, pC0E0Arr, tValues, dataPath, lines, argumentR, argumentP, minProb,mainPath, plottingPath):
    # preparing the subplot
    fig, axes = plt.subplots(len(pC0E0Arr), len(priorE0Arr), sharex= True, sharey= True)
    fig.set_size_inches(16, 16)
    fig.set_facecolor("white")
    ...
    for cueVal in pC0E0Arr: # for each cue validity
        ...
        for pE0 in priorE0Arr: # for each prior
            ...
            for t in tValues:
                ...
                # add a column highlighting rows in which organisms deconstruct
                aggregatedResultsDF['deconstruct'] = ['k' if ('2' in str(elem) or '3' in str(elem)) else 'none' for elem in aggregatedResultsDF['cStar']]
                ...
                if t== 1:
                    ...
                    subDF = subDF.reset_index(drop=True)
                    circles(np.array(subDF['time']), np.array(subDF['newpE1']),
                            s=np.array(subDF['area']), ax=ax, c=subDF['colors'], zorder=2,
                            ec=list(subDF['deconstruct']), lw=0.7, alpha=0.8)
                ...
                if t >= 1:
                    ...
                    if duplicateList:
                        stateProbs = list(subDF['stateProb'])
                        decisionMarker = list(subDF['marker'])
                        edgeColors = list(subDF['deconstruct'])
                        ...
                        for key in duplicateList:
                            ...
                            stateProbPies.append([stateProbs[i] for i in idxDuplList])
                            decisionsPies.append([decisionMarker[i] for i in idxDuplList])
                            edgecolorPies.append([edgeColors[i] for i in idxDuplList])
                        ...
                        circles(np.array(newSubDf['time']), np.array(newSubDf['newpE1']),
                                s=np.array(newSubDf['area']), ax=ax, c=newSubDf['colors'], zorder=2,
                                ec=list(newSubDf['deconstruct']), lw=0.7, alpha=0.8)
                    else:
                        circles(np.array(subDF['time']), np.array(subDF['newpE1']),
                                s=np.array(subDF['area']), ax=ax, c=subDF['colors'], zorder=2,
                                ec=list(subDF['deconstruct']), lw=0.7, alpha=0.8)
            ...
            for idx in range(len(coordinates)):
                ...
                for idy, wedge in enumerate(pp):
                    wedge.set_edgecolor(edgecolorPies[idx][idy])
                    wedge.set_linewidth(0.6)
                    wedge.set_alpha(0.7)

                [p.set_zorder(4) if edgecolorPies[idx][idy] == 'k' else p.set_zorder(3) for idy,p in enumerate(pp)] #+ len(coordinates) - idx
                ax.set_frame_on(True)
            ...
            if jX == 0:
                plt.ylabel('posterior', fontsize=20, labelpad=10)
    ...
    fig.subplots_adjust(top=0.94, right=0.91)
    #plt.savefig(os.path.join(resultPath,'DevelopmentalTrajectory.pdf'), dpi = 900)
    plt.savefig(os.path.join(resultPath, 'DevelopmentalTrajectoryReduced_simplified.jpg'), bbox_inches='tight' ,dpi=1200)
