...
from PlottingParallel import joinIndidividualResultFiles,policyPlotReduced_v2Paper, policyPlotReduced_v2
...
def func_star(allArgs):
    return itertools.product(*allArgs)

def func_star2(allArgs):
    T = allArgs[1]
    resultsCurr = [flatten_tuples(curr) for curr in allArgs[0]]
    phenotypesCurr = list(
        ((y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw, D0, D1) for y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw, D0, D1 in
         resultsCurr if
         # list all the constraints here
         (sum([y0BC, y1BC, y0AC, y1AC, y0D, y1D, yw]) == T) and
         ((y0BC >= y0D and y0BC <= D0 - y0D) if y0D > 0 else (
                 y0BC == 0 and D0 == 0)) and  # if y0D >0 else they are 0
         ((y1BC >= y1D and y1BC <= D1 - y1D) if y1D > 0 else (
                 y1BC == 0 and D1 == 0)) and  # if y1D > 0 else they are 0
         ((y0AC <= T - D0) if D1 < D0  else ((y0AC <= T - D0 - 1) if D1 > 0 else (y0AC <= T - D0))) and
         ((y1AC <= T - D1) if D0 < D1 else ((y1AC <= T - D1 - 1) if D0 > 0 else (y1AC <= T - D1))) and
         ((D0 >= 2 * y0D) and (D1 >= 2 * y1D + 2 * y0D) if D0 < D1 else (D0 >= 2 * y0D + 2 * y1D) and (
                 D1 >= 2 * y1D))
         ))
    return phenotypesCurr
...
def flatten_tuples(t):
    for x in t:
        if isinstance(x, tuple):
            ...
            yield x
def calcTerminalFitness(tree_path, y0BA,y1BA, y0Deconstruct, y1Deconstruct,
                        ywValues, D0, D1, T, beta, argumentR, argumentP, psi_weightingParam):
    ...
    # first compute the possible phenotypic states
    # first prefilter these phenotypes
    # only combinations of y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw == T
    preCombinations = itertools.product(*[y0BA, y1BA, y0Deconstruct, y1Deconstruct,
                                          ywValues])
    preCombinationsFiltered = list((y0BA, y1BA, y0D, y1D, yw) for y0BA, y1BA, y0D, y1D, yw in preCombinations if
                                   sum([*y0BA, *y1BA, y0D, y1D, yw]) == T)

    pool = Pool(32)

    results = pool.map(func_star,
                       zip([[curr] for curr in preCombinationsFiltered], itertools.repeat(D0), itertools.repeat(D1)))
    ...
    phenotypes = []
    pool = Pool(32)
    phenotypesResults = pool.map(func_star2, zip(list(results), itertools.repeat(T)))

    pool.close()
    pool.join()

    for res in phenotypesResults:
        if len(res) != 0:
            phenotypes += res

    # next, combine these phenotypic states with the possible cues that could have been sampled
    for D in trees.keys():
        x0, x1 = D
        posE0, posE1 = trees[D][0]

        for y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw, D0_curr,D1_curr in phenotypes:
            ...
            terminalF[currKey] = terminalFitness((x0, x1, y0AC, y1AC, yw), posE0,
                                                 posE1, argumentR,
...
# @profile
def F(x0, x1, y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw, D0_curr,D1_curr, posE0, posE1,
    ...
    if prFT != 0:
        TF = prFT * terminalFitness((x0, x1, y0AC, y1AC, yw), posE0, posE1,
    ...
    if t == T - 1:  # is this is the last decision and we have seen all cues
        ...
        currKey1 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
            x0, x1, y0BC, y0AC + 1, y1BC, y1AC, y0D, y1D, yw, D0_curr, D1_curr, t1)

        currKey2 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
            x0, x1, y0BC, y0AC, y1BC, y1AC + 1, y0D, y1D, yw, D0_curr, D1_curr, t1)

        if y0AC > y0D:  # equivalent to saying that you cannoy deconstruct below 0

            currKey3 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
                x0, x1, y0BC + y0AC, 0, y1BC, y1AC, y0D + 1, y1D, yw, t1, D1_curr, t1)
        else:
            currKey3 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
                x0, x1, y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw + 1, D0_curr, D1_curr, t1)

        if y1AC > y1D:
            currKey4 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
                x0, x1, y0BC, y0AC, y1BC + y1AC, 0, y0D, y1D + 1, yw, D0_curr, t1, t1)
        else:
            currKey4 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
                x0, x1, y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw + 1, D0_curr, D1_curr, t1)

        currKey5 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
            x0, x1, y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw + 1, D0_curr, D1_curr, t1)
        ...
        if (isinstance(maxDecision, tuple)):
            if 2 in maxDecision and int(currKey3.split(";")[8]) > yw:
                # find the index of 2
            ...
            if 2 == maxDecision and int(currKey3.split(";")[8]) > yw:
                maxDecision = 4

        if (isinstance(maxDecision, tuple)):
            if 3 in maxDecision and int(currKey4.split(";")[8]) > yw:
                maxDecision = list(maxDecision)
                # find the index of 3
            ...
            if 3 == maxDecision and int(currKey4.split(";")[8]) > yw:
                maxDecision = 4

    else:
        currKey1 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
            x0 + 1, x1, y0BC, y0AC + 1, y1BC, y1AC, y0D, y1D, yw, D0_curr, D1_curr, t1)
        currKey2 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
            x0, x1 + 1, y0BC, y0AC + 1, y1BC, y1AC, y0D, y1D, yw, D0_curr, D1_curr, t1)

        currKey3 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
            x0 + 1, x1, y0BC, y0AC, y1BC, y1AC + 1, y0D, y1D, yw, D0_curr, D1_curr, t1)
        currKey4 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
            x0, x1 + 1, y0BC, y0AC, y1BC, y1AC + 1, y0D, y1D, yw, D0_curr, D1_curr, t1)

        if y0AC > y0D:
            currKey5 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
                x0 + 1, x1, y0BC + y0AC, 0, y1BC, y1AC, y0D + 1, y1D, yw, t1, D1_curr, t1)
            currKey6 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
                x0, x1 + 1, y0BC + y0AC, 0, y1BC, y1AC, y0D + 1, y1D, yw, t1, D1_curr, t1)

        else:
            currKey5 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
                x0 + 1, x1, y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw + 1, D0_curr, D1_curr, t1)
            currKey6 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
                x0, x1 + 1, y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw + 1, D0_curr, D1_curr, t1)

        if y1AC > y1D:
            currKey7 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
                x0 + 1, x1, y0BC, y0AC, y1BC + y1AC, 0, y0D, y1D + 1, yw, D0_curr, t1, t1)
            currKey8 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
                x0, x1 + 1, y0BC, y0AC, y1BC + y1AC, 0, y0D, y1D + 1, yw, D0_curr, t1, t1)
        ...
        currKey9 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
            x0 + 1, x1, y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw + 1, D0_curr, D1_curr, t1)
        currKey10 = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
            x0, x1 + 1, y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw + 1, D0_curr, D1_curr, t1)
        ...
        if (isinstance(maxDecision, tuple)):
            if 2 in maxDecision and int(currKey5.split(";")[8]) > yw:
                # find the index of 2
                maxDecision = list(maxDecision)
                i = maxDecision.index(2)
                maxDecision = maxDecision[:i] + [4] + maxDecision[i + 1:]
            ...
            if 2 == maxDecision and int(currKey5.split(";")[8]) > yw:
                maxDecision = 4

        if (isinstance(maxDecision, tuple)):
            if 3 in maxDecision and int(currKey7.split(";")[8]) > yw:
                maxDecision = list(maxDecision)
                # find the index of 3
                i = maxDecision.index(3)
                maxDecision = maxDecision[:i] + [4] + maxDecision[i + 1:]
            ...
            if 3 == maxDecision and int(currKey7.split(";")[8]) > yw:
                maxDecision = 4
...
def oneFitnessSweep(tree, t, phenotypes, finalTimeArr, T, argumentR, argumentP):
    ...
    for D in tree.keys():
        ...
        for phenotype in phenotypes:
            y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw, D0_curr, D1_curr = phenotype
            #y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw, D0_curr,D1_curr
            fitness, optimalDecision = F(x0, x1, y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw, D0_curr,D1_curr, posE0, posE1,
                                         pC1D, pC0D, t, argumentR, argumentP, T,
                                         finalTimeArr)

            currKey = '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s' % (
            x0, x1, y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw, D0_curr, D1_curr, t)
...
def run(priorE0, cueValidityC0E0, T, argumentR, argumentP, kwUncertainTime, beta,
    ...
    # narrow it down to valid combinations of yA and yB
    yAValues = np.arange(0, T + 1, 1)
    yBValues = np.arange(0, T + 1, 1)

    yBA = list((yB,yA) for yB,yA in itertools.product(*[yBValues,yAValues]) if sum([yB,yA]) <= T)

    # number of times deconstructed on either dimension
    y0Deconstruct = np.arange(0, round(T / 2) + 1, 1)
    y1Deconstruct = np.arange(0, round(T / 2) + 1, 1)
    # number of times waited
    ywValues = np.arange(0, T + 1, 1)


    # phenotype identifiers
    # they keep track of the last time period during which either phenotypic dimension has been deconstructed
    D0 = np.arange(0, T + 1, 1)
    D1 = np.arange(0, T + 1, 1)
    ...
    terminalFitness = calcTerminalFitness(tree_path, yBA,yBA, y0Deconstruct,
                                          y1Deconstruct,
                                          ywValues, D0, D1, T, beta, argumentR, argumentP,
                                          psi_weightingParam)
    ...
    for t in tValues:
        ...
        # we will adjust this every time to speed up the process
        yAValues = np.arange(0, t + 1, 1)
        yBValues = np.arange(0, t + 1, 1)
        ...
        #goal: get all possible phenotypes in the current time period
        preCombinations = itertools.product(*[yBA, yBA, y0Deconstruct, y1Deconstruct,
                                              ywValues])
        preCombinationsFiltered = list((y0BA, y1BA, y0D, y1D, yw) for y0BA, y1BA, y0D, y1D, yw in preCombinations if
                                       sum([*y0BA, *y1BA, y0D, y1D, yw]) == t)
        ...
        results = pool.map(func_star,
                           zip([[curr] for curr in preCombinationsFiltered], itertools.repeat(D0),
                               itertools.repeat(D1)))
        ...
        # get the phenotypes from the parallel processing
        for res in phenotypesResults:
            if len(res) != 0:
                phenotypes += res

        del results
        del phenotypesResults
...
if __name__ == '__main__':
    ...
    argumentRArr = ['linear'] #'linear','diminishing','increasing'
    argumentPArr = ['linear'] #'linear','diminishing','increasing'
    ...
    if plotFlag:
        ...
        for argumentR in argumentRArr:
            for argumentP in argumentPArr:
                ...
                r = 0.7  # this is the radius baseline for the policy plots np.sqrt(1/float(np.pi))#0.5
                ...
                for startEnv in startEnvList:
                    ...
                    # policyPlotReduced_v2Paper(int(T) + 1, r, priorE0Arr, cueValidityC0E0Arr,
                    ...
                    # policyPlotReduced_v2(int(T) + 1, 0.5, priorE0Arr, cueValidityC0E0Arr,
                    #                  np.arange(1, int(T) + 1, 1), dataPath2, True,
                    #                  argumentR, argumentP, minProb,
                    #                  mainPath, twinResultsPath)
                    ...
                    #              startEnv)
                    # #
                    # plotArgs = ["ExperimentalTwinstudy"]  # "Twinstudy","BeliefTwinstudy", "ExperimentalTwinstudy"]
                    # for adoptionType in ["yoked"]:  # , "oppositePatch","deprivation"]:
                    #     print("adoptionType: " + str(adoptionType))
                    #     runPlots(priorE0Arr, cueValidityC0E0Arr, int(T), numAgents,
                    #              twinResultsPath,
