...
def updatePhenotype(subDF, phenotypeTracker, simValues, cueTracker, t):
    optDecisions = [subDF.loc[(subDF['x0'] == cueTracker[idx, 0]) & (subDF['x1'] == cueTracker[idx, 1]) & (
            subDF['y0BC'] == phenotypeTracker[idx, 0]) & (subDF['y0AC'] == phenotypeTracker[idx, 1]) & (
                                      subDF['y1BC'] == phenotypeTracker[idx, 2]) & (
                                      subDF['y1AC'] == phenotypeTracker[idx, 3]) & (
                                      subDF['y0D'] == phenotypeTracker[idx, 4]) & (
                                      subDF['y1D'] == phenotypeTracker[idx, 5]) & (
                                      subDF['yw'] == phenotypeTracker[idx, 6]) & (
                                      subDF['D0_curr'] == phenotypeTracker[idx, 7]) & (
                                      subDF['D1_curr'] == phenotypeTracker[idx, 8])]['cStar'].item() for idx in
                    simValues]
    ...
    posBelief = [subDF.loc[(subDF['x0'] == cueTracker[idx, 0]) & (subDF['x1'] == cueTracker[idx, 1]) & (
            subDF['y0BC'] == phenotypeTracker[idx, 0]) & (subDF['y0AC'] == phenotypeTracker[idx, 1]) & (
                                   ...
                                   subDF['yw'] == phenotypeTracker[idx, 6]) & (
                                   subDF['D0_curr'] == phenotypeTracker[idx, 7]) & (
                                   subDF['D1_curr'] == phenotypeTracker[idx, 8])]['pE1'].item() for idx in
                 simValues]
    ...
    if len(idx0) > 0:
        phenotypeTracker[idx0, 1] += 1  # construction y0

    idx1 = [idx for idx, val in enumerate(optDecisionsNum) if val == 1]
    if len(idx1) > 0:
        phenotypeTracker[idx1, 3] += 1  # construction y1

    idx2 = [idx for idx, val in enumerate(optDecisionsNum) if val == 2]  # deconstruct 0
    if len(idx2) > 0:
        idx2_Dpossible = [idx for idx in idx2 if phenotypeTracker[idx, 1] > phenotypeTracker[idx, 4]]  # y0AC > y0D
        idx2_Dimpossible = [idx for idx in idx2 if phenotypeTracker[idx, 1] <= phenotypeTracker[idx, 4]]

        phenotypeTracker[idx2_Dpossible, 4] += 1  # deconstruction y0
        phenotypeTracker[idx2_Dpossible, 0] = phenotypeTracker[idx2_Dpossible, 0] + phenotypeTracker[idx2_Dpossible, 1]
        phenotypeTracker[idx2_Dpossible, 1] = 0
        phenotypeTracker[idx2_Dpossible, 7] = t  # update the last time deconstruction happened
        phenotypeTracker[idx2_Dimpossible, 6] += 1  # if we cannot deconstruct, we wait

    # 'y0BC', 'y0AC', 'y1BC', 'y1AC', 'y0D', 'y1D', 'yw','D0_curr', 'D1_curr'
    idx3 = [idx for idx, val in enumerate(optDecisionsNum) if val == 3]  # deconstruct 1
    if len(idx3) > 0:
        idx3_Dpossible = [idx for idx in idx3 if phenotypeTracker[idx, 3] > phenotypeTracker[idx, 5]]  # y1AC > y1D
        idx3_Dimpossible = [idx for idx in idx3 if phenotypeTracker[idx, 3] <= phenotypeTracker[idx, 5]]

        phenotypeTracker[idx3_Dpossible, 5] += 1  # deconstruction y0
        phenotypeTracker[idx3_Dpossible, 2] = phenotypeTracker[idx3_Dpossible, 2] + phenotypeTracker[idx3_Dpossible, 3]
        phenotypeTracker[idx3_Dpossible, 3] = 0
        phenotypeTracker[idx3_Dpossible, 8] = t  # update the last time deconstruction happened

        phenotypeTracker[idx3_Dimpossible, 6] += 1  # if we cannot deconstruct, we wait

    idx4 = [idx for idx, val in enumerate(optDecisionsNum) if val == 4]
    if len(idx4) > 0:
        phenotypeTracker[idx4, 6] += 1  # waiting
...
def simulateExperimentalTwins(tAdopt, twinNum, env, cueReliability, lag, T, adoptionType, endOfExposure):
    ...
    cueTracker[:, 0] = 1 - cues
    cueTracker[:, 1] = cues
    phenotypeTracker = np.zeros((twinNum, 9))
    ...
    for t in tValues:
        ...
        phenotypeTracker, __ = updatePhenotype(subDF, phenotypeTracker, simValues, cueTracker, t)
        if t < T:
            # next cue sequences
    ...
    for t2 in restPeriod:
        ...
        originalTwin, __ = updatePhenotype(subDF, originalTwin, simValues, cueTracker, t2)

        doppelgaenger, __ = updatePhenotype(subDF, doppelgaenger, simValues, cueTrackerDoppel, t2)
        if t2 < T:
    ...
    if not endOfExposure:  # this means we want to measure phenotypic distance at the end of onotgeny
        for t3 in restPeriodReunited:
            ...
            originalTwin, __ = updatePhenotype(subDF, originalTwin, simValues, cueTracker, t3)
            doppelgaenger, __ = updatePhenotype(subDF, doppelgaenger, simValues, cueTrackerDoppel, t3)
...
def simulateTwins(tAdopt, twinNum, env, adopt, T, adoptionType, pE0, cueVal):
    ...
    if adopt:
        ...
        cueTracker[:, 0] = 1 - cues
        cueTracker[:, 1] = cues
        phenotypeTracker = np.zeros(
            (twinNum, 9))  # 'y0BC', 'y0AC', 'y1BC', 'y1AC', 'y0D', 'y1D', 'yw','D0_curr', 'D1_curr'
        ...
        for t in tValues:
            ...
            phenotypeTracker, posBeliefTracker = updatePhenotype(subDF, phenotypeTracker, simValues, cueTracker, t)
            if t < T:
                cues = [int(cue) for cue in allCues[:, t]]
                cues = np.array(cues)
        ...
        for t2 in restPeriod:  # this is where adoption starts
            ...
            # update the phenotypes of the twins
            originalTwin, posBeliefOrg = updatePhenotype(subDF, originalTwin, simValues, cueTracker, t2)

            posBeliefTrackerOrg[:, t2 - tAdopt + 1] = posBeliefOrg
            doppelgaenger, posBeliefDG = updatePhenotype(subDF, doppelgaenger, simValues, cueTrackerDoppel, t2)
        ...
        phenotypeTracker = np.zeros((twinNum, 9))
        phenotypeTrackerTemporal = np.zeros((twinNum, 9, T))
        ...
        for t in tValues:
            ...
            # print identTracker
            phenotypeTracker, posBelief = updatePhenotype(subDF, phenotypeTracker, simValues, cueTracker, t)
...
def calcEuclideanDistance(original, doppelgaenger, idxTuple):
    if len(idxTuple) == 2:
        ...
        result = [np.sqrt(np.sum((x - y) ** 2)) for x, y in
                  zip(original[:, [idx1, idx2]], doppelgaenger[:, [idx1, idx2]])]

    else:
        original0 = original[:, idxTuple[0]] + original[:, idxTuple[1]]
        original1 = original[:, idxTuple[2]] + original[:, idxTuple[3]]
        doppelgaenger0 = doppelgaenger[:, idxTuple[0]] + doppelgaenger[:, idxTuple[1]]
        doppelgaenger1 = doppelgaenger[:, idxTuple[2]] + doppelgaenger[:, idxTuple[3]]
        result = [np.sqrt(np.sum((x - y) ** 2)) for x, y in
                  zip(np.column_stack((original0, original1)), np.column_stack((doppelgaenger0, doppelgaenger1)))]
...
def runExperimentalAdoptionExperiment(T, numAgents, env, prior, cueReliability, resultsPath, argumentR, argumentP, lag,
    ...
    allResults = {}
    namedList = {'y0y1': (1, 3), 'y0Cy1C': (0, 1, 2, 3), 'y0Dy1D': (4, 5)}
    ...
    for t in tValues:
        ...
        for key, val in namedList.items():
            ...
            else:  # need to add up the number of constructions before and after the last deconstruction
                if atypicalIDX == 0:
                    idx1 = val[0]
                    idx11 = val[1]
                    idx2 = val[2]
                    ...
                    idx2 = val[0]
                    idx22 = val[1]
                atypicalSpec = zip(list((original[:, idx1] + original[:, idx11])),
                                   list((doppelgaenger[:, idx1] + doppelgaenger[:, idx11])))
                typicalSpec = zip(list((original[:, idx2] + original[:, idx22])),
                                  list((doppelgaenger[:, idx2] + doppelgaenger[:, idx22])))
    ...
    for t in tValues:
        ...
        for key, val in namedList.items():
            ...
            if len(val) == 2:
                ...
                if atypicalIDX == 0:
                    idx1 = val[0]  # number of specializations towards / deconstructions of atypical env
                    idx11 = val[1]  # number of specializations towards / deconstructions of atypical env
                    idx2 = val[2]  # # number of specializations towards / deconstructions of typical env
                    idx22 = val[3]
                else:
                    idx1 = val[2]  # number of specializations towards / deconstructions of atypical env
                    idx11 = val[3]  # number of specializations towards / deconstructions of atypical env
                    idx2 = val[0]  # # number of specializations towards / deconstructions of typical env
                    idx22 = val[1]
                atypicalSpec = zip(list((original[:, idx1] + original[:, idx11])),
                                   list((doppelgaenger[:, idx1] + doppelgaenger[:, idx11])))
                typicalSpec = zip(list((original[:, idx2] + original[:, idx22])),
        ...
        if not endOfExposure:  # if phenotypic distance has been measured at the end of ontogeny
            ...
            for idx in range(results.shape[0]):
                ...
                if key == 'y0Dy1D':  # need to think about rounding
                    curRowNorm = curRow / float(((lag + idx) / 2) * np.sqrt(2))
                    curRowRelative = curRow / float((lag / 2) * np.sqrt(2))
                else:
...
def plotTriangularPlots(tValues, priorE0Arr, cueValidityArr, maturePhenotypes, T, twinResultsPath, env):
    ...
    namedList = {'y0y1': (1, 3), 'y0Cy1C': (0, 1, 2, 3), 'y0Dy1D': (4, 5)}
    namedListColors = {'y0y1': ('black', 1, 300), 'y0Cy1C': ('blue', 0.3, 500), 'y0Dy1D': ('green', 0.4, 500)}
    ...
    for cueVal in cueValidityArr:
        ...
        for pE0 in priorE0Arr:
                ...
                if len(val) == 2:
                    maturePhenotypesCurr = np.transpose(
                        [maturePhenotypesCurrAll[:, val[0]], maturePhenotypesCurrAll[:, val[1]],
                         maturePhenotypesCurrAll[:, 6]])
                else:

                    maturePhenotypesCurr = np.transpose(
                        [maturePhenotypesCurrAll[:, val[0]] + maturePhenotypesCurrAll[:, val[1]],
                         maturePhenotypesCurrAll[:, val[2]] + maturePhenotypesCurrAll[:, val[3]],
                         maturePhenotypesCurrAll[:, 6]])
            ...
            #     ax.set_zlabel('waiting', fontsize=fontsize, rotation='vertical', labelpad=10)
            #
            #     ax.set_zticks(np.arange(0, max(tValues) + 1, 2), labels, fontsize=13, rotation=0)
            ...
            # else:
            #     ax.set_zticks(np.arange(0, max(tValues) + 1, 2), labels, fontsize=0, rotation=0)
            #     ax.set_xticks(np.arange(0, max(tValues) + 1, 2), labels, fontsize=0, rotation=0)
            #     ax.set_yticks(np.arange(0, max(tValues) + 1, 2), labels, fontsize=0, rotation=0)
            #
            # ax.set_aspect('equal')
...
def plotTriangularPlotsArrow(tValues, priorE0Arr, cueValidityArr, maturePhenotypes, T, twinResultsPath, env):
    ...
    # namedList = {'y0y1': (5, 6), 'y0Cy1C': (0, 1), 'y0Dy1D': (2, 3)}
    namedList = {'y0y1': (1, 3), 'y0Cy1C': (0, 1, 2, 3), 'y0Dy1D': (4, 5)}
    namedListColors = {'y0y1': ('black', 1, 300), 'y0Cy1C': ('blue', 0.3, 300), 'y0Dy1D': ('green', 0.4, 300)}
    ...
    for cueVal in cueValidityArr:
        ...
        for pE0 in priorE0Arr:
            ...
            noDPhen = uniqueCompletePhen[np.where((uniqueCompletePhen[:,4] == 0)*(uniqueCompletePhen[:,5] == 0))]

            noDIDX = [idx for idx in range(len(uniqueCompletePhen)) if uniqueCompletePhen[idx,:].tolist() in noDPhen.tolist() ]

            percentD = round((1 - (sum(counts[noDIDX])/float(numAgents)))*100,2)

            noDPhen = noDPhen[:,[1,3,6]]

            #plotting construction and total phenotypes
            #starting with construction
            noDIDX = [idx for idx in range(len(constructionPhen)) if constructionPhen[idx,:].tolist() in noDPhen.tolist() ]
            DIDX = [idx for idx in range(len(constructionPhen)) if constructionPhen[idx,:].tolist() not in noDPhen.tolist() ]
            ...
            # draw arrows
            uniqueCompletePhenX = uniqueCompletePhen[:, [1, 3]] #total phenotype
            uniqueCompletePhenY = np.transpose([uniqueCompletePhen[:, 0] + uniqueCompletePhen[:, 1],
                         uniqueCompletePhen[:, 2] + uniqueCompletePhen[:, 3]]) # construction steps
            ...
            [ax.annotate('', [uniqueCompletePhenX[idx][0], uniqueCompletePhenX[idx][1]],
                         [uniqueCompletePhenY[idx][0], uniqueCompletePhenY[idx][1]], arrowprops={'arrowstyle': "->",
                                                                                                 ...
                                                                                                 'shrinkB': 3,
                                                                                                 'zorder':1}) for idx
             in range(len(uniqueCompletePhen))]
    ...
    #plt.savefig(os.path.join(twinResultsPath, 'ternary2D_%s.pdf' % env), dpi=600)
    plt.savefig(os.path.join(twinResultsPath, 'ternary2D_%s.png' % env), dpi=600)

    plt.close()


def plotTriangularPlotsNew(tValues, priorE0Arr, cueValidityArr, maturePhenotypes, T, twinResultsPath, env):
    ...
    for cueVal in cueValidityArr:
        ...
        for pE0 in priorE0Arr:
            ...
            numAgents = maturePhenotypesCurrAll.shape[0]
            uniqueCompletePhen, counts = np.unique(maturePhenotypesCurrAll, axis=0, return_counts=True)
            noDPhen = uniqueCompletePhen[np.where((uniqueCompletePhen[:,4] == 0)*(uniqueCompletePhen[:,5] == 0))]
            ...
            # what do we need
            # format: y0BC, y0AC, y1BC, y1AC, y0D, y1D, yw,
            # change in the following way: y0C = y0BC + y0AC and so on
            maturePhenotypesCurrAll = np.transpose(
                [sum([maturePhenotypesCurrAll[:, 0], maturePhenotypesCurrAll[:, 1]]),
                 sum([maturePhenotypesCurrAll[:, 2], maturePhenotypesCurrAll[:, 3]]),
                 maturePhenotypesCurrAll[:, 4],
                 maturePhenotypesCurrAll[:, 5],
                 maturePhenotypesCurrAll[:, 6]
                 ])

            maturePhenotypesCurr = np.transpose(
                [maturePhenotypesCurrAll[:, 0], maturePhenotypesCurrAll[:, 1],
                 sum([maturePhenotypesCurrAll[:, 4], maturePhenotypesCurrAll[:, 2], maturePhenotypesCurrAll[:, 3]])
                 ])
            ...
            # get colors based on the unique phenotypes
            # for each Y0C, Y1C check back in maturePhenotypesAll

            # filter all rows based on some of the column values


            waitColors = ['black' if sum(maturePhenotypesCurrAll[np.where(
                (maturePhenotypesCurrAll[:, 0] == unique[idx, 0]) * (maturePhenotypesCurrAll[:, 1] == unique[idx, 1]))][
                                             ..., 4]) > 0 else 'none' for idx in range(len(unique))]

            # if Y0D >0 and Y1D =0, color = light red
            redIDX = [idx for idx in range(len(unique)) if (sum(maturePhenotypesCurrAll[np.where(
                (maturePhenotypesCurrAll[:, 0] == unique[idx, 0]) * (maturePhenotypesCurrAll[:, 1] == unique[idx, 1]))][
                                             ..., 2]) == 0 and sum(maturePhenotypesCurrAll[np.where(
                (maturePhenotypesCurrAll[:, 0] == unique[idx, 0]) * (maturePhenotypesCurrAll[:, 1] == unique[idx, 1]))][
                                             ..., 3]) > 0)]
            # if Y0D = 0 and Y1D > 0, color = light blue
            blueIDX = [idx for idx in range(len(unique)) if (sum(maturePhenotypesCurrAll[np.where(
                (maturePhenotypesCurrAll[:, 0] == unique[idx, 0]) * (maturePhenotypesCurrAll[:, 1] == unique[idx, 1]))][
                                                                    ..., 2]) > 0 and sum(
                maturePhenotypesCurrAll[np.where(
                    (maturePhenotypesCurrAll[:, 0] == unique[idx, 0]) * (
                                maturePhenotypesCurrAll[:, 1] == unique[idx, 1]))][
                    ..., 3]) == 0)]


            # if Y0D >0 and Y1D >0, color = light pruple
            purpleIDX = [idx for idx in range(len(unique)) if (sum(maturePhenotypesCurrAll[np.where(
                (maturePhenotypesCurrAll[:, 0] == unique[idx, 0]) * (maturePhenotypesCurrAll[:, 1] == unique[idx, 1]))][
                                                                    ..., 2]) > 0 and sum(
                maturePhenotypesCurrAll[np.where(
                    (maturePhenotypesCurrAll[:, 0] == unique[idx, 0]) * (
            ...
            #next, create the actual color list
            circleColors = []
            for idx in range(len(unique)):
                if idx in redIDX:
                    circleColors.append('#FF8D79')
                elif idx in blueIDX:
                    circleColors.append('#79C4FF')
                elif idx in purpleIDX:
                    circleColors.append('#d39beb')
                else:
                    circleColors.append('#9c7248')

            # area = area_calc(uniqueCounts / float(numAgents), 150)
            area2 = (uniqueCounts / float(numAgents)) * 450
            # this one would be scalling according to area


            tax.scatter(unique[:, permuteIdx], s=area2, color= circleColors, edgecolors=None)
            ...
            ax.set_xlabel("deconstruction: %s%%" % percentD, fontsize=20, labelpad=-5)

            if jx == len(priorE0Arr) - 1:
                plt.ylabel(str(cueVal), labelpad=10, rotation='vertical', fontsize=20)
                ax.yaxis.set_label_position("right")

            if jx == int(len(priorE0Arr) / 2) and ix == int(len(cueValidityArr) / 2):
                fontsize = 20
                tax.right_corner_label(" P0", fontstyle = 'italic',fontsize=fontsize,    )
                tax.top_corner_label("wait & deconstruct", fontstyle = 'italic',fontsize=fontsize)
                tax.left_corner_label("P1 ", fontstyle = 'italic',fontsize=fontsize)
                tax._redraw_labels()
            jx += 1
        ix += 1
        fig.text(0.47, 0.98, 'prior probability', fontsize=20, horizontalalignment='center', verticalalignment='center')

        fig.text(0.98, 0.49, 'cue reliability', fontsize=20, horizontalalignment='right', verticalalignment='center',
                 rotation='vertical')
    plt.subplots_adjust(left=0, bottom=0.05, top=0.92, right=0.92)

    plt.savefig(os.path.join(twinResultsPath, 'ternary_%sNew.png' % env), dpi=600)
    plt.close()
    ...
    for cueVal in cueValidityC0E0Arr:
        ...
        for pE0 in priorE0Arr:
            ...
            if arg == 'atypical':
                [plt.plot(tValues, keyVal[1], linestyle=linestyle_tuple[idx][1], linewidth=2, markersize=5,
                          marker='o', color='black', markerfacecolor='black',
                          label="%s O" % (keyVal[0])) for idx, keyVal in enumerate(atypicalTraceOrg.items()) if
                 keyVal[0] != 'y0y1']
                [plt.plot(tValues, keyVal[1], linestyle=linestyle_tuple[idx][1], linewidth=2, markersize=5,
                          marker='o', color='grey', markerfacecolor='grey', label="%s C" % (keyVal[0])) for idx, keyVal
                 in enumerate(atypicalTraceClone.items()) if keyVal[0] != 'y0y1']
                ...
                [plt.plot(tValues, keyVal[1], linestyle=linestyle_tuple[idx][1], linewidth=2, markersize=5,
                          marker='o', color='black', markerfacecolor='black',
                          label="%s O" % (keyVal[0])) for idx, keyVal in enumerate(typicalTraceOrg.items()) if
                 keyVal[0] != 'y0y1']
                [plt.plot(tValues, keyVal[1], linestyle=linestyle_tuple[idx][1], linewidth=2, markersize=5,
                          marker='o', color='grey', markerfacecolor='grey',
                          label="%s C" % (keyVal[0])) for idx, keyVal in enumerate(typicalTraceClone.items()) if
                 keyVal[0] != 'y0y1']
            ...
            plt.ylim(-0.05, max(tValues) + 0.5)
            plt.yticks(np.arange(0, max(tValues) + 1, 2), fontsize=15)
            plt.xticks(np.arange(1, max(tValues) + 1, 2), fontsize=15)

            if ix == len(cueValidityC0E0Arr) - 1 and jx == 0:
                anchPar = len(priorE0Arr) / float(2)
                legend = ax.legend(bbox_to_anchor=(0.2, -0.5, 1., .102), loc='lower left',
                                   title='phenotypic dimensions',
                                   ncol=4, fancybox=True, shadow=False, fontsize=20)
                plt.setp(legend.get_title(), fontsize='20')
            if ix == 0:
                plt.title("%s" % (1 - pE0), fontsize=20)
...
def postProcessTraces(atypicalTraces, typicalTraces, cueProbabilities):
    atypicalTraceOrg = {}
    atypicalTraceClone = {}
    typicalTraceOrg = {}
    typicalTraceClone = {}
    ...
    if argument == "ExperimentalTwinstudy":
        ...
        for pE0 in priorE0Arr:
            for cueReliability in cueValidityC0E0Arr:
                ...
                resultsMat, cueProbabilities, atypicalTraces, typicalTraces = runExperimentalAdoptionExperiment(T,
                                                                                                                numAgents,
                                                                                                                env,
                                                                                                                pE0,
                                                                                                                cueReliability,
        ...
        for pE0 in priorE0Arr:
            for cueReliability in cueValidityC0E0Arr:
                ...
                tValues = np.arange(1, T + 1, 1)
                for t in tValues:
                    possibleRanks = sorted(list(set(current[:, 3, t - 1])), reverse=True)
                    currentMat[:, t - 1] = [possibleRanks.index(a) + 1 for a in current[:, 3,
...
def plotSimulationStudy(argument, priorE0Arr, cueValidityC0E0Arr, T, twinResultsPath, lag, adoptionType,
    ...
    if argument == "BeliefTwinstudy":
        ...
        plotTriangularPlotsNew(tValues, priorE0Arr, cueValidityC0E0Arr, maturePhenotypesResults, T,
                            twinResultsPath, env)
